...
// 2) if -> логическое И (if без else)
// 3) if -> return <boolean expression>

// 4) for -> for of (если arr[i] только для чтения (не мутируем массив))
// 5) String(233) -> `${233}`
// 6) Number('10') -> +'10'
// 7) Boolean('text') -> !!'text'

// ? Разбор 3)
// Написать функцию, которая проверят переданное число на чётность
function isEven(num) {
  // if (num % 2 === 0) {
  //   return true
  // } else {
